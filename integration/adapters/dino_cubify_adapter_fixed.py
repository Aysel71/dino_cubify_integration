...
class DINOCubifyAdapter(nn.Module):
    ...
    def __init__(
        ...
        # DINO encoder - используем меньше слоев
        # ViT-B обычно имеет 12 слоев (0-11)
        self.dino_encoder = DINOv2Encoder(
            backbone=backbone,
            intermediate_layers=[11],  # Используем только последний слой
            dim_out=embed_dim
        )
...
def test_adapter():
    """Test the adapter"""
    print("Testing fixed DINO-CubifyAnything adapter...")
    ...
    if torch.cuda.is_available():
        ...
        print(f"GPU memory used: {torch.cuda.max_memory_allocated() // 1024**2} MB")
